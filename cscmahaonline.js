...
window.addEventListener("DOMContentLoaded", function () {
    ...
    const dashboard = document.getElementById("aepsdashboard");

    if (!isAdmin()) {

        dashboard.removeAttribute("href");

        dashboard.classList.add("disabled-link");

        dashboard.title = "Administrator user can access AEPS Dashboard";
...
for(let i=1;i<=pages;i++){
    ...
    doc.text(
        "Generated by DHANADAI ENTERPRISES | CSC | MahaOnline Daily Cash Management System",
        10,
        206
    );
